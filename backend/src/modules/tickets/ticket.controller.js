    ...
    try {
        ...
        for (const admin of admins) {
            await pool.query(
                ...
                [admin.id, ticketId, `Novo chamado #${ticketId} aberto.`]
            );
        }
    ...
    try {
        ...
        if (status && status !== ticket.status) {
            ...
            if (ticket.requester_id !== req.user.id) {
                await pool.query(
                    'INSERT INTO notifications (user_id, ticket_id, message) VALUES (?, ?, ?)',
                    [ticket.requester_id, ticketId, `Status do chamado #${ticketId} alterado para ${status}.`]
                );
            }
        ...
        if (req.user.role === 'admin' && assigned_to !== undefined && assigned_to !== ticket.assigned_to) {
            ...
            if (assigned_to) {
                await pool.query(
                    'INSERT INTO notifications (user_id, ticket_id, message) VALUES (?, ?, ?)',
                    [assigned_to, ticketId, `Chamado #${ticketId} atribuído a você.`]
                );
            }
        }
...
export const addObservation = async (req, res) => {
    ...
    try {
        ...
        const usersToNotify = new Set([ticket.requester_id, ticket.assigned_to].filter(id => id && id !== req.user.id));
        for (const userId of usersToNotify) {
            await pool.query(
                'INSERT INTO notifications (user_id, ticket_id, message) VALUES (?, ?, ?)',
                [userId, ticketId, `Nova mensagem no chamado #${ticketId}.`]
            );
        }
